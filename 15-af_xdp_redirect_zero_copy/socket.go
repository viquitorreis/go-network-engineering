...
func newSocket(ifindex, queueID int) (*Socket, error) {
	...
	if err := unix.Bind(fd, sa); err != nil {
		// we could check for errors ENOTSUP/EOPNOTSUPP, however this will vary from kernel to kernel and NIC
		slog.Warn("zero-copy bind failed, falling back to copy mode", "error", err)
		sa = &unix.SockaddrXDP{
			Flags:   unix.XDP_COPY,
			Ifindex: uint32(ifindex),
			QueueID: uint32(queueID),
		...
		if err := unix.Bind(fd, sa); err != nil {
			return nil, fmt.Errorf("bind XDP_COPY AF_XDP socket (fallback): %w", err)
		}

		slog.Info("AF_XDP socket bound in copy mode")
		return sock, nil
	}

	slog.Info("AF_XDP socket bound in zero-copy mode")
	return sock, nil
}
