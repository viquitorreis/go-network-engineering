...
func buildFrame(seq uint32, payload uint16, timestamp uint64) []byte {
	msg := protocol.RTPDatagram{SeqNumber: seq, Payload: payload, Timestamp: timestamp}
	body := msg.ToDatagram()
...
func TestReceptor_InOrderPackets_NoGapsNoDelays(t *testing.T) {
	...
	for seq := uint32(0); seq < 5; seq++ {
		conn.Write(buildFrame(seq, uint16(seq), uint64(seq)*20))
	}
	time.Sleep(50 * time.Millisecond)

	if len(r.SeqGaps) != 0 {
		t.Errorf("esperava 0 gaps em sequência sem perda, teve %d", len(r.SeqGaps))
	}
	if len(r.Delayed) != 0 {
		t.Errorf("esperava 0 pacotes atrasados em sequência sem perda, teve %d", len(r.Delayed))
	}
	if r.LastSeqNum != 4 {
		t.Errorf("LastSeqNum = %d, esperado 4 (último seq recebido)", r.LastSeqNum)
	}
}
...
func TestReceptor_DetectsGap(t *testing.T) {
	...
	if len(r.SeqGaps) != 1 {
		t.Fatalf("esperava 1 evento de gap detectado, teve %d", len(r.SeqGaps))
	}
	if r.SeqGaps[0].SeqNumber != 4 {
		t.Errorf("gap deveria estar associado ao seq 4 (o que revelou o buraco), veio seq %d", r.SeqGaps[0].SeqNumber)
	}
	if r.LastSeqNum != 4 {
		t.Errorf("LastSeqNum deveria avançar para 4 mesmo com gap, veio %d", r.LastSeqNum)
	}
}
...
func TestReceptor_DetectsOutOfOrderPacket(t *testing.T) {
	...
	if len(r.Delayed) != 1 {
		t.Fatalf("esperava 1 pacote atrasado detectado, teve %d", len(r.Delayed))
	}
	if r.Delayed[0].SeqNumber != 1 {
		t.Errorf("pacote atrasado deveria ser o seq 1, veio seq %d", r.Delayed[0].SeqNumber)
	}
	if r.LastSeqNum != 2 {
		t.Errorf("LastSeqNum não deveria retroceder por causa de um pacote atrasado, veio %d", r.LastSeqNum)
	}
}
