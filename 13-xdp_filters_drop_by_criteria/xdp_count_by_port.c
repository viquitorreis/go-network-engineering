...
// each protocol that runs on top of Ethernet have a reserved number/address
// 0x0800 = IPv4
// 0x0806 = ARP
// 0x86DD = IPv6
...
struct {
    ...
    __type(key, __u16);   // destiny port
    __type(value, __u64); // counting
} port_count SEC(".maps"); // will be objs.PortCount
...
struct {
    ...
    __type(key, __u16);   // destiny port
    __type(value, __u64); // counting
} blocked_ports SEC(".maps"); // will be objs.BlockedPorts
...
SEC("xdp")
int count_by_port(struct xdp_md *ctx) { // will become objs.CountByPort
    ...
    // only processes ipv4, ignores IPv6, ARP, etc
    ...
    // checks if port is blocked, must check the blocked_ports map
    // if it is blocked we drop the package
    __u64 *blocked = bpf_map_lookup_elem(&blocked_ports, &port);
    if (blocked) {
        __sync_fetch_and_add(blocked, 1); // increments the drop counter
        return XDP_DROP;
    } 
