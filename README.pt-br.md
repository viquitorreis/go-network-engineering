...
| 15 | [XDP_REDIRECT + Negociação de Zero-Copy: Modo de Bind Explícito com Fallback de Driver](./15-af_xdp_redirect_zero_copy) | Networking / Kernel Bypass | Estende o socket AF_XDP do challenge 14 com negociação explícita de modo de bind: tenta XDP_ZEROCOPY primeiro, cai pra XDP_COPY em erros de driver não suportado em vez de deixar o kernel escolher silenciosamente, e loga qual modo ficou ativo de fato; testado em hardware real (NIC r8169, confirmada sem suporte a zero-copy), com o caminho de fallback validado de ponta a ponta recebendo pacote real. |
