...
| 10 | [Jitter Buffer: Reordering](./27-jitter_buffer_reordering) | Streaming / Networking | Reuses the skip list built for the order book (same O(log n) insert / O(1) front, now keyed by sequence number instead of price) to hold out-of-order RTP-style packets until either their scheduled release time arrives or a per-packet deadline expires; a ticker drains the front of the list into a playback channel, so a packet that never shows up gets skipped instead of stalling playback, and the player reconstructs expected-sequence gaps to report definitive loss and playback latency. |
