...
| 10 | [Jitter Buffer: Reordering](./10-jitter_buffer_reordering) | Streaming / Networking | Reuses the skip list built for the order book (same O(log n) insert / O(1) front, now keyed by sequence number instead of price) to hold out-of-order RTP-style packets until either their scheduled release time arrives or a per-packet deadline expires; a ticker drains the front of the list into a playback channel, so a packet that never shows up gets skipped instead of stalling playback, and the player reconstructs expected-sequence gaps to report definitive loss and playback latency. |
| 11 | [UDP FEC (XOR): Sequence Packets + Parity + Reconstruction](./11-udp_fec_via_xor_seq_parity_packets) | Streaming / Networking | Adds proactive forward error correction on top of the RTP-style packetizer: the sender XORs every fixed-size group of data packets into a parity packet sent alongside them, and the receiver reconstructs a single missing packet per group from the survivors plus parity, reporting definitive loss only when 2 or more packets in the same group are missing, since simple XOR parity cannot recover more than one loss per group. |
| 12 | [XDP: Program Counts Packets by Port](./12-xdp_program_counter_by_port) | Networking / Kernel bypass | First kernel-bypass challenge: a restricted-C program attached at the XDP hook parses Ethernet/IP/UDP headers by hand and increments a per-destination-port counter in an eBPF map, entirely before the kernel allocates an sk_buff; a Go userspace loader (cilium/ebpf + bpf2go) loads the verified bytecode, attaches it to an interface, and polls the same map for live counts, with the two sides communicating only through shared kernel-managed memory. |
| 13 | [XDP: Filter/Drop by Criteria](./13-xdp_filters_drop_by_criteria) | Networking / Kernel bypass | Extends the packet counter with a second eBPF map holding a Go-controlled blocklist, so the kernel-side program drops matching packets via XDP_DROP before they cost any further kernel processing, while the userspace loader updates the policy at runtime with no recompile; the split mirrors the real production pattern (Cilium, Katran) of a userspace control plane deciding policy that a kernel data plane only executes, plus a caught bug where packets were misread as IPv4 without checking eth->h_proto first, silently corrupting parsing for any IPv6 traffic. |
| 14 | [AF_XDP Socket: First Attach — Custom XDP Redirect via bpf2go](./14-af_xdp_socket) | Networking / Kernel Bypass | Attaches a custom C XDP program (compiled via bpf2go, same pattern as challenges 12/13) that redirects packets into an XSKMAP-registered AF_XDP socket; the userspace side is hand-written Go using raw syscalls against golang.org/x/sys/unix (UMEM registration, fill/rx ring setup via mmap, bind) rather than a third-party AF_XDP wrapper, trading the fixed hardcoded program of libraries like asavie/xdp for full control over the redirect logic. |
| 15 | [XDP_REDIRECT + Zero-Copy Negotiation: Explicit Bind Mode with Driver Fallback](./15-af_xdp_redirect_zero_copy) | Networking / Kernel Bypass | Extends the AF_XDP socket from challenge 14 with explicit bind-mode negotiation: tries XDP_ZEROCOPY first, falls back to XDP_COPY on driver-unsupported errors instead of letting the kernel silently choose, and logs which mode actually ends up active; tested on real hardware (r8169 NIC, which confirmed lacks zero-copy support), with the fallback path validated end-to-end receiving live packets. |
